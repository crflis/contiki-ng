/*
 * Link log output helper for the embedded RPL border router.
 *
 * Collects log output into a line buffer, then emits it as a framed message
...
flush_line(void)
{
  ...
  br_link_write(frame, line_len + 2);

  line_len = 0;
}

void
br_log_output(const char *fmt, ...)
{
